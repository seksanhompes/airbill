...
// .wrangler/tmp/pages-NcPx9y/functionsWorker-0.39610950294104597.mjs
...
async function onRequestGet({ request, env }) {
  ...
  const binds = [];
  if (person_id) {
    where.push("p.person_id = ?");
    binds.push(person_id);
  }
  if (from) {
...
async function onRequestPut({ request, env, params }) {
  ...
  if (typeof body.amount !== "undefined") {
    fields.push("amount=?");
    binds.push(Number(body.amount) || 0);
  }
  if (typeof body.note !== "undefined") {
    fields.push("note=?");
    binds.push(String(body.note || ""));
  }
  if (typeof body.paid_on !== "undefined") {
    fields.push("paid_on=?");
    binds.push(String(body.paid_on));
  }
...
async function onRequestPost({ request, env }) {
  ...
  const ts = Date.now();
  await env.DB.prepare(`INSERT INTO payments (id,person_id,amount,paid_on,note,created_at) VALUES (?,?,?,?,?,?)`).bind(id, person_id, amount, paid_on, note, ts).run();
...
// .wrangler/tmp/bundle-0eeMCd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0eeMCd/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.39610950294104597.js.map
